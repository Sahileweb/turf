...
for (const cb of cancelledBookings) {
  if (cb.payment) {
    await tx.payment.delete({ where: { id: cb.payment.id } })
  }
  await tx.booking.delete({ where: { id: cb.id } })
}
...
const booking = await tx.booking.create({
  ...
  return res.status(400).json({
    success: false,
    message: 'This slot has already started or passed. Please choose a future slot.'
  })
}
