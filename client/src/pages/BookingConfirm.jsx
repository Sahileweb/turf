...
//   // ── statusRef is ALWAYS in sync because we update it synchronously ──
//   // This eliminates the timing window where ref and state disagree
//   const statusRef = useRef('idle')

//   // ── Helper: always update BOTH ref and state together ──
//   const updateStatus = (newStatus) => {
//     statusRef.current = newStatus  // Synchronous — happens immediately
//     setStatus(newStatus)           // Async — triggers re-render
//   }
...
//       // Only cancel if:
//       // 1. Was genuinely mounted (not StrictMode fake unmount)
//       // 2. Payment was NOT completed or in progress
//       if (
//         bookingId &&
//         wasReallyMounted &&
...
//     updateStatus('paying')   // ← use updateStatus, not setStatus
//     setError('')
...
//       modal: {
//         ondismiss: () => {
//           updateStatus('idle')   // ← use updateStatus
//         }
//       }
//     }
...
//       updateStatus('failed')   // ← use updateStatus
...
//     // ── CRITICAL: update ref synchronously FIRST before any async call ──
//     // This guarantees cleanup will never cancel a booking that's being confirmed
...
//       updateStatus('success')   // ← use updateStatus
...
const BookingConfirm = () => {
  ...
  // statusRef always mirrors status synchronously
  // Used in cleanup to avoid stale closure
  const statusRef = useRef('idle')
  ...
  // This runs on REAL unmount (not StrictMode — we removed it)
  useEffect(() => {
    return () => {
      // Only cancel if payment was never started or failed
      // 'success' and 'confirming' mean payment is done or in progress
      if (
        bookingId &&
  ...
  const confirmPayment = async (razorpayPaymentId, razorpayOrderId, razorpaySignature) => {
    // Set 'confirming' synchronously BEFORE the async call
    // This ensures cleanup never cancels a booking that's being confirmed
